...
namespace cheat::feature
{
    ...
    {
        static const FeatureGUIInfo info{ u8"×Ô¶¯µöÓã", "World", true };
        return info;
    }

    void AutoFish::DrawMain()
    {
        ConfigWidget(u8"¿ª¹Ø", f_Enabled, u8"×Ô¶¯µöÓã.");
        ConfigWidget(u8"×¥È¡ÑÓ³Ù (ms)", f_DelayBeforeCatch, 100, 500, 4000, u8"Ìá¸ËµÄµÄÑÓ³Ù (in ms).");

        ImGui::Spacing();

        ConfigWidget(f_AutoRecastRod, u8"×Ô¶¯Å×¸Í.");
        ConfigWidget(u8"Å×¸ÍÑÓ³Ù (ms)", f_DelayBeforeRecast, 10, 100, 4000, u8"Å×¸ÍÑÓ³Ù (in ms).");
    }
